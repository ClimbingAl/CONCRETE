...
<details markdown="block">
<summary>What's involved in a space-time diagram?</summary>

These diagrams cane be created using a tool (a prototype of which is available online[^9]).

They are visual representations of what's involved in the activities of interest and the activities themselves.  While the look is similar to a project plan, they are conveying what material things are involved as participants in one or more of the activities being shown.  They include the following:

- An axis for "space".  Items extended on this axis, principally the horizontal grey bars, represent physical things that have unique identity in the 'real' world (e.g. a particular UAV, a person, a functional system like a UAV detection and surveilance system, etc).  They occupy some space.
- An axis for "time". Items extended on this axis exist for a period of time.  Usually there is no scale in the time axis as the primary purpose is to show the order of changes relative to other changes.
- Physical things that exist (or, for modelling purposes, could exist as shown) are extended in space and time.  They are shown as horizontal grey bars.
- Activities, that involve physical things as participants for a period of time, are cloured boxes that enclose states of the physical things that are participants in a particular activity.
